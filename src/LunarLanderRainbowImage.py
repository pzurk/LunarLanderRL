...
def dqn(n_episodes=200000, max_t=1000, eps=1.0, eps_end=0.01, eps_decay=0.9998):
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for t in range(max_t):
            ...
            agent.step(state, action, reward, next_state, terminated or truncated, beta)
            state = next_state
            score += reward
            if done:
                break
        ...
        print(f"Episode {i_episode}  Score: {score:.2f}  Avg100: {np.mean(scores_window):.2f}  eps: {eps:.3f}  beta:{beta:.3f}")
